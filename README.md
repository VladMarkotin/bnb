...
ВАЖНО!
Для изменения параметров подключения к бд, перейдите в каталог core/db/ в файл init_file.php измените представленные там параметры. Если подключение будет производиться не к локальному хосту, то в файле core/db/DBClass.php в методе init() нужно изменить строку "new \PDO('mysql:host=127.0.0.1:3306;dbname=' . DB, USER, PASS);", заменив <mysql:host=127.0.0.1:3306> на mysql:host="your host":"your port"
